...
import UnweighedDetailsTable from './UnweighedDetailsTable'; // <-- 1. IMPORT BẢNG MỚI
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3">
        {isLoading ? (
          <AdminPageSkeleton /> // Dùng Skeleton
        ) : data.length > 0 ? (
          data.map(group => (
            // 3. THÊM Fragment VÀ LOGIC HIỂN THỊ CHI TIẾT
            <React.Fragment key={group.ovNO}>
              <div onClick={() => handleSelectOvno(group.ovNO)} className="cursor-pointer">
                <UnweighedSummaryCard data={group} />
              </div>

              {/* Hiển thị chi tiết nếu OVNO này được chọn */}
              {selectedOvno === group.ovNO && (
                <div className="pl-4 pr-4 pb-2">
                  {isDetailsLoading ? (
                    <p className="text-center text-gray-500 p-4">Đang tải chi tiết...</p>
                  ) : (
                    <UnweighedDetailsTable data={details} />
                  )}
                </div>
              )}
            </React.Fragment>
